...
int main()
{
...
#if !defined(UNICODE)
	strcpy_s(g_lpPackageName, 1024 * sizeof(CHAR), "Negotiate");
#else
	wcscpy_s(g_lpPackageName, 1024 * sizeof(CHAR), L"Negotiate");
#endif
	...
	{
		fprintf(stderr,
			"Could not query package info for %ws, error 0x%08x\n",
			g_lpPackageName, ss);
		cleanup();
	}
	...
	while (TRUE)
	{
		...
		pUserName = (PWCHAR)calloc(cbUserName + 1, sizeof(WCHAR));
		//memset(pUserName, 0, cbUserName + 1);
		if (!pUserName)
		{
			fprintf(stderr, "Memory allocation error. \n");
			cleanup();
