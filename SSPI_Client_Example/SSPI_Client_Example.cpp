...
#define TargetName  "Domain\username"
...
int main()
{
	...
	{
		MyHandleError((char *)"Could not initialize winsock ");
	}
	...
	if (!ConnectAuthSocket(
		...
		&hcText))
	{
		MyHandleError((char *)"Authenticated server connection ");
	}
	...
	if (!SEC_SUCCESS(ss))
	{
		MyHandleError((char *)"QueryContextAttributes failed ");
	}
	else
	{
	...
	if (!SEC_SUCCESS(ss))
	{
		MyHandleError((char *)"Query context ");
	}
	...
	if (!ReceiveBytes(
		...
		&cbRead))
	{
		MyHandleError((char *)"No response from server ");
	}

	if (0 == cbRead)
	{
		MyHandleError((char *)"Zero bytes received ");
	}
	...
	{
		MyHandleError((char *)"Problem with socket cleanup ");
	}
	...
	struct _SecHandle *hcText)
{
	...
	if (INADDR_NONE == ulAddress)
	{
		...
		if (NULL == pHost)
		{
			MyHandleError((char *)"Unable to resolve host name ");
		}
	...
	*s = socket(
	...
	if (INVALID_SOCKET == *s)
	{
		MyHandleError((char *)"Unable to create socket");
	}
	else
	{
	...
	{
		closesocket(*s);
		MyHandleError((char *)"Connect failed ");
	}
	...
	{
		closesocket(*s);
		MyHandleError((char *)"Authentication ");
	}
...

{
	...
	{
		MyHandleError((char *)"Memory allocation ");
	}
	...
	{
		MyHandleError((char *)"Send message failed ");
	}
	...
	while (!fDone)
	{
		if (!ReceiveMsg(
			...
			&cbIn))
		{
			MyHandleError((char *)"Receive message failed ");
		}
		...
		if (!GenClientContext(
			...
			&hcText))
		{
			MyHandleError((char *)"GenClientContext failed");
		}
		if (!SendMsg(
			s,
			pOutBuf,
			cbOut))
		{
			MyHandleError((char *)"Send message 2  failed ");
		}
	}
	...
	struct _SecHandle *hcText)
{
	...
	if (NULL == pIn)
	{
		...
		if (!(SEC_SUCCESS(ss)))
		{
			MyHandleError((char *)"AcquireCreds failed ");
		}
	}
	...
	if (!SEC_SUCCESS(ss))
	{
		MyHandleError((char *)"InitializeSecurityContext failed ");
	}
